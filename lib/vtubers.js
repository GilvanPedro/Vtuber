...
const CAMPOS_PUBLICOS = `
    ...
    v.seguidores_twitch AS "seguidoresTwitch", v.inscritos_youtube AS "inscritosYoutube",
...
export async function salvarEstatisticas(id, { twitch, youtube, falhas = {} }) {
    await sql(
        `UPDATE vtubers SET
            ...
            estatisticas_em = CASE WHEN $2 OR $4 THEN estatisticas_em ELSE now() END
         WHERE id = $1`,
        [id, Boolean(falhas.twitch), twitch ?? null, Boolean(falhas.youtube), youtube ?? null]);
}

// Vtubers com link de Twitch ou YouTube, as mais desatualizadas primeiro.
...
export async function vtubersComRedes({ desatualizadasHaHoras = null } = {}) {
    return sql(
        `SELECT id, redes FROM vtubers
          WHERE (redes ? 'twitch' OR redes ? 'youtube')
