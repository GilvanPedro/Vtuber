...
const CAMPOS_PUBLICOS = `
    v.id, v.nome, v.cor, v.tags, v.horario, v.plataforma, v.idioma, v.agenda, v.fuso,
    v.horario_fusos AS "horarioFusos",
    v.seguidores_twitch AS "seguidoresTwitch", v.inscritos_youtube AS "inscritosYoutube", v.seguidores_kick AS "seguidoresKick",
...
export async function criarVtuber(vt, criadoEm) {
    return salvar([
        [`INSERT INTO vtubers (id, nome, cor, tags, horario, plataforma, idioma, bio, bio_en, redes, videos,
                                agenda, fuso, horario_fusos, criado_em)
          VALUES ($1, $2, $3, $4::jsonb, $5::jsonb, $6::jsonb, $7::jsonb, $8, $9, $10::jsonb, $11::jsonb,
                  $12::jsonb, $13, $14::jsonb, COALESCE($15::timestamptz, now()))`,
            [vt.id, vt.nome, vt.cor, j(vt.tags), j(vt.horario), j(vt.plataforma), j(vt.idioma),
                vt.bio, vt.bioEn, j(vt.redes), j(vt.videos), j(vt.agenda), vt.fuso, j(vt.horarioFusos ?? {}), criadoEm ?? null]],
...
export async function atualizarVtuber(idAtual, vt) {
    return salvar([
        [`UPDATE vtubers SET id = $2, nome = $3, cor = $4, tags = $5::jsonb, horario = $6::jsonb,
             ...
             videos = $12::jsonb, agenda = $13::jsonb, fuso = $14, horario_fusos = $15::jsonb, atualizado_em = now()
          WHERE id = $1`,
            [idAtual, vt.id, vt.nome, vt.cor, j(vt.tags), j(vt.horario), j(vt.plataforma), j(vt.idioma),
                vt.bio, vt.bioEn, j(vt.redes), j(vt.videos), j(vt.agenda), vt.fuso, j(vt.horarioFusos ?? {})]],
        ...await consultasDeImagens(vt.id, vt.imagens)
    ], vt.id);
}
...
// Grava os números das redes. Se a consulta de uma plataforma falhou, mantém o valor que já estava salvo
// e não marca como atualizada (assim ela é tentada de novo na próxima vez).
export async function salvarEstatisticas(id, { twitch, youtube, kick, falhas = {} }) {
    await sql(
        `UPDATE vtubers SET
            seguidores_twitch = CASE WHEN $2 THEN seguidores_twitch ELSE $3::integer END,
            inscritos_youtube = CASE WHEN $4 THEN inscritos_youtube ELSE $5::integer END,
            seguidores_kick   = CASE WHEN $6 THEN seguidores_kick ELSE $7::integer END,
            estatisticas_em   = CASE WHEN $2 OR $4 OR $6 THEN estatisticas_em ELSE now() END
         WHERE id = $1`,
        [id, Boolean(falhas.twitch), twitch ?? null, Boolean(falhas.youtube), youtube ?? null,
            Boolean(falhas.kick), kick ?? null]);
}

// Vtubers com link de Twitch, YouTube ou Kick, as mais desatualizadas primeiro.
// Com "desatualizadasHaHoras", só as que nunca foram consultadas ou foram há mais tempo que isso.
export async function vtubersComRedes({ desatualizadasHaHoras = null } = {}) {
    return sql(
        `SELECT id, redes FROM vtubers
          WHERE (redes ? 'twitch' OR redes ? 'youtube' OR redes ? 'kick')
            AND ($1::float IS NULL OR estatisticas_em IS NULL OR estatisticas_em < now() - $1::float * interval '1 hour')
          ORDER BY estatisticas_em NULLS FIRST`,
        [desatualizadasHaHoras]);
}
