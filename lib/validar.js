...
// Fusos horários aceitos (precisam bater com FUSOS em js/fusos.js)
export const FUSOS = ['America/Sao_Paulo', 'America/New_York', 'America/Los_Angeles', 'Europe/Paris'];
const MAX_HORARIOS_AGENDA = 14;
...
// Agenda: [{ dias: [0..6], inicio: 'HH:MM', fim: 'HH:MM' | null }] (horários no fuso da vtuber)
function validarAgenda(agenda) {
    if (agenda == null) return [];
    if (!Array.isArray(agenda)) falhar('"agenda" precisa ser uma lista.');
    if (agenda.length > MAX_HORARIOS_AGENDA) falhar(`A agenda pode ter até ${MAX_HORARIOS_AGENDA} horários.`);
    const hora = valor => /^([01]\d|2[0-3]):[0-5]\d$/.test(valor ?? '');
    return agenda.map((item, i) => {
        const dias = [...new Set(item?.dias ?? [])].map(Number).sort((a, b) => a - b);
        if (!dias.length) falhar(`Horário ${i + 1} da agenda: escolha pelo menos um dia.`);
        if (dias.some(d => !Number.isInteger(d) || d < 0 || d > 6)) falhar(`Horário ${i + 1} da agenda: dia da semana inválido.`);
        if (!hora(item.inicio)) falhar(`Horário ${i + 1} da agenda: hora de início inválida.`);
        if (item.fim && !hora(item.fim)) falhar(`Horário ${i + 1} da agenda: hora de término inválida.`);
        return { dias, inicio: item.inicio, fim: item.fim || null };
    });
}
...
export function validarVtuber(corpo, { opcoes }) {
    ...
    const fuso = corpo.fuso ?? FUSOS[0];
    if (!FUSOS.includes(fuso)) falhar('Fuso horário inválido.');

    // Períodos definidos à mão para outros fusos (sem "diverso", que vale para todos)
    const horarioFusos = {};
    for (const [outroFuso, periodos] of Object.entries(corpo.horarioFusos ?? {})) {
        if (!FUSOS.includes(outroFuso)) falhar(`Fuso horário inválido: ${outroFuso}`);
        const periodosValidos = lista(periodos, 'horario', HORARIOS.filter(h => h !== 'diverso'));
        if (outroFuso !== fuso && periodosValidos.length) horarioFusos[outroFuso] = periodosValidos;
    }

    return {
        id, nome, cor, bio, bioEn, redes, videos, fuso, horarioFusos,
        agenda: validarAgenda(corpo.agenda),
