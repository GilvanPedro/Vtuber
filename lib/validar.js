...
// As opções precisam bater com FILTROS em js/vtubers-data.js.
const OPCOES = {
    tags: ['just-chatting', 'gameplay', 'react', 'asmr', 'musica', 'arte'],
...
function lista(valor, grupo) {
    if (!Array.isArray(valor)) falhar(`"${grupo}" precisa ser uma lista.`);
    const invalidos = valor.filter(v => !OPCOES[grupo].includes(v));
...
export function validarVtuber(corpo) {
    ...
    const bio = String(corpo.bio || '').replace(/\r\n/g, '\n').trim();
    if (bio.length > 6000) falhar('A bio passa de 6000 caracteres.');
    ...
    return {
        id, nome, cor, bio, redes, videos,
        tags: lista(corpo.tags ?? [], 'tags'),
