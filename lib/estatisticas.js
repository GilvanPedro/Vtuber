...
// ---------- Kick ----------
// A API oficial da Kick (api.kick.com) não informa seguidores; usamos a rota que o próprio site da Kick usa.
// Não precisa de chave. Por não ser oficial, pode mudar: se falhar, o último número salvo é mantido.
export function slugDaKick(link) {
    try {
        const url = new URL(link);
        if (!/(^|\.)kick\.com$/.test(url.hostname)) return null;
        const slug = url.pathname.split('/').filter(Boolean)[0]?.toLowerCase();
        return /^[a-z0-9_-]{2,25}$/.test(slug || '') && !['video', 'category', 'categories', 'search'].includes(slug) ? slug : null;
    } catch {
        return null;
    }
}

// O número vem como "followersCount" (v1) ou "followers_count" (v2), às vezes como texto ("1115443").
function numeroDeSeguidoresKick(dados) {
    const valor = Number(dados?.followersCount ?? dados?.followers_count);
    return Number.isFinite(valor) && valor >= 0 ? valor : null;
}

export async function seguidoresNaKick(link) {
    const slug = slugDaKick(link);
    if (!slug) return null;
    let ultimoErro;
    for (const versao of ['v1', 'v2']) { // a v1 tem se mostrado mais estável; a v2 fica de reserva
        try {
            const resposta = await fetch(`https://kick.com/api/${versao}/channels/${encodeURIComponent(slug)}`, {
                headers: { Accept: 'application/json', 'User-Agent': 'Mozilla/5.0 (compatible; VtuberSearch/1.0)' },
                signal: AbortSignal.timeout(TEMPO_LIMITE_MS)
            });
            if (resposta.status === 404) return null; // canal não existe
            if (!resposta.ok) throw new Error(`kick.com (${versao}) respondeu ${resposta.status}`);
            const total = numeroDeSeguidoresKick(await resposta.json());
            if (total != null) return total;
            throw new Error(`kick.com (${versao}) não informou os seguidores`);
        } catch (e) {
            ultimoErro = e;
        }
    }
...
export async function estatisticasDasRedes(redes = {}) {
    const [twitch, youtube, kick] = await Promise.allSettled([
        ...
        redes.kick ? seguidoresNaKick(redes.kick) : null
    ]);
    for (const [nome, resultado] of [['da Twitch', twitch], ['do YouTube', youtube], ['da Kick', kick]]) {
    ...
    return {
        ...
        falhas: { twitch: falhou(twitch), youtube: falhou(youtube), kick: falhou(kick) }
    };
}
...
export async function atualizarVarias({ desatualizadasHaHoras = null, limiteMs = Infinity } = {}) {
    ...
    const trabalhar = async () => {
        for (let vt = fila.shift(); vt && Date.now() < prazo; vt = fila.shift()) {
            ...
            if (r.falhas?.erro || ['twitch', 'youtube', 'kick'].some(rede => vt.redes[rede] && r.falhas?.[rede])) falhas.push(vt.id);
            else atualizadas++;
        }
    };
